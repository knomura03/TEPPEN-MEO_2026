...
## 21. P3-04 順位/競合ダッシュボード 実装方針
### 21.1 目的
- 直近runの順位推移と競合比較を、画面上で即座に把握できるようにする
- P3-02/P3-03で蓄積したデータを「運用判断に使える形」で可視化する

### 21.2 DB変更（migration）
- 追加migrationなし（P3-02/P3-03の既存テーブルを再利用）
- 利用テーブル:
  - `rank_collection_runs`
  - `rank_collection_results`
  - `competitor_metric_snapshots`
...
### 21.3 Service/UI変更
- `services/rankCollectionService.ts`
  - `listRunDetailsByStore(storeId, limit)` を追加
  - runごとの順位結果/競合結果を束ねて返し、UI側のN+1ロジックを隠蔽する
- `components/RankTrackerView.tsx`
  - 「順位/競合ダッシュボード（P3-04）」セクションを追加
  - 可視化内容:
    - 最新runサマリー（平均順位、最上位キーワード、競合平均順位）
    - 主要キーワードの順位推移チャート
    - 最新runの競合順位バー比較
    - run別サマリーテーブル（順位平均/競合平均/件数）
  - 競合テーブル未適用時はダッシュボード全体を落とさず、警告表示で継続する

### 21.4 実装ルール（固定）
- ダッシュボードは「店舗単位」のみを対象にする（右上店舗セレクタ依存）
- 集計値は保存せず、既存runデータから都度計算する
- 競合未設定/0件時は空状態メッセージを表示し、エラー扱いしない
- `REAL` / `MOCK` の区別なく保存済みデータを同一可視化に載せる
