...
const parseCredentialPayload = (encoded: string): { accessToken: string; credentialPayload: Record<string, unknown> } => {
  try {
...
Deno.serve(async (req) => {
  ...
  if (!canPublish) {
    return jsonResponse(403, { error: 'Only ADMIN/MANAGER can publish to Instagram' });
  }
