...
1. TEPPEN MEOにログインする
2. 右上の店舗セレクタで対象店舗を選ぶ
3. 「プラットフォーム管理」で対象providerの接続状態を確認する
...
### 10-2. 店舗未割当ユーザーの復旧（GUI）
1. ADMIN/SUPERVISOR/MANAGERでログインし、「ユーザー管理」を開く
2. 対象ユーザーに対して、店舗を「割当」または「新規作成」する
3. 対象ユーザーでログインし、右上の店舗セレクタで店舗が選択できることを確認する

期待結果:
- 手動SQLなしで、ユーザーに店舗を割当できる
- 割当後に店舗セレクタへ反映される
...
### 12-2. OAuth開始
1. ADMIN/SUPERVISOR/MANAGER/USER でログインする
2. 「プラットフォーム管理」を開く
3. OAUTH2 provider（Facebook/Instagram/GBP）の「連携する」を押す
4. OAuthモーダルの案内に従って認可画面を開き、許可する

期待結果:
- OAuthモーダルが開く
- 認可URLが表示される（認可コード貼り付けは不要）
...
### 19-1. migration確認（必須）
1. Supabase SQL Editorで `supabase/migrations/202602060017_p3_competitor_comparison_collection.sql` を実行する
2. `Success. No rows returned` になることを確認する

期待結果:
- `competitor_targets` / `competitor_metric_snapshots` が作成される

### 19-2. 競合ターゲット登録
1. 右上の店舗セレクタで対象店舗を選ぶ
2. 左メニューの「順位計測」を開く
3. 「競合ターゲット（P3-03）」で競合名を入力して「競合を追加」を押す
...
### 19-3. 収集実行と競合結果確認
1. 同画面で「収集実行（MOCK）」を押す
2. 実行履歴から最新runを選択する
3. 「競合比較」結果を確認する

期待結果:
- 競合ごとの順位/口コミ数/評価が表示される
- runメッセージに順位件数と競合件数が記録される

### 19-4. 異常系
1. migration未適用環境で競合ターゲットを追加する
2. 競合ターゲット0件のまま収集を実行する

期待結果:
- migration未適用時は専用エラーを表示する
- 競合0件でも収集自体は成功し、競合0件として記録される

## 20. P3-04 順位/競合ダッシュボード

### 20-1. 前提データ
1. `P3-02` と `P3-03` が適用済みであることを確認する
2. 対象店舗で収集runを1回以上実行する
...
1. 右上の店舗セレクタで対象店舗を選ぶ
2. 左メニューの「順位計測」を開く
3. 「順位/競合ダッシュボード（P3-04）」を確認する

期待結果:
- 最新runサマリー（平均順位/最上位キーワード/競合平均順位）が表示される
- キーワード推移チャートが表示される
- 競合データがある場合、競合比較グラフが表示される
...
### 20-4. 異常系
1. runが0件の店舗を選択する
2. 競合ターゲット0件の店舗で収集runを表示する

期待結果:
- run0件では空状態メッセージが表示される
- 競合0件では競合比較だけ空表示になり、画面全体はエラーにならない

## 21. P3-05 NAP整合性チェック

### 21-1. migration確認（必須）
1. Supabase SQL Editorで `supabase/migrations/202602060018_p3_nap_consistency_check.sql` を実行する
2. `Success. No rows returned` になることを確認する

期待結果:
- `nap_consistency_runs` / `nap_consistency_results` が作成される

### 21-2. チェック実行
1. 右上の店舗セレクタで対象店舗を選ぶ
2. 左メニューの「順位計測」を開く
3. 「NAP整合性チェック（P3-05）」で「NAPチェック実行」を押す

期待結果:
- 履歴に新しいrunが追加される
- サマリー（MATCH/MISMATCH/MISSING）が表示される

### 21-3. 結果確認
1. 履歴から最新runを選ぶ
2. Providerごとの詳細を確認する

期待結果:
- `MATCH` は一致として表示される
- `MISMATCH` は不一致項目（name/address/phone）が表示される
- `MISSING` は未設定として表示される

### 21-4. 異常系
1. provider設定が未入力の店舗で実行する
2. 店舗NAPの一部を空欄にして実行する

期待結果:
- provider未設定は `MISSING` で記録される（run失敗にはしない）
- NAP欠損時も例外終了せず、比較可能な範囲で結果が記録される

## 22. P3-06 NAPアラート運用

### 22-1. migration確認（必須）
1. Supabase SQL Editorで `supabase/migrations/202602060019_p3_nap_alert_operations.sql` を実行する
2. `Success. No rows returned` になることを確認する

期待結果:
- `nap_alerts` テーブルが作成される

### 22-2. アラート同期確認
1. 右上の店舗セレクタで対象店舗を選ぶ
2. 左メニューの「順位計測」を開く
3. `P3-05` の「NAPチェック実行」を押す
4. 「NAPアラート（P3-06）」の一覧を確認する

期待結果:
- `MISMATCH/MISSING` があるproviderはアラートとして表示される
- `MATCH` のproviderは未対応一覧に表示されない

### 22-3. ACK/解消操作
1. アラートがあるproviderで `ACK` を押す
2. 再読込しても `ACKED` のまま維持されることを確認する
3. 同じアラートで `解消` を押す

期待結果:
- `ACK` で `OPEN -> ACKED` に遷移する
- `解消` で未対応一覧から消える

失敗時の確認:
- `P3-06 migration（NAP alert operations）の適用後に再試行してください。` の場合は migration 未適用（ただしP3-05は継続可能）
