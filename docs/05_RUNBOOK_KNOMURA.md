...
## （P3-04）順位/競合ダッシュボードを使う手順
「順位計測」画面で推移チャートと比較表を使うには、P3-02/P3-03のデータが必要です（追加migrationは不要）。

### 手順（クリック順）
1. TEPPEN MEOでログインする
2. 右上の店舗セレクタで対象店舗を選ぶ
3. 左メニューの「順位計測」を開く
4. 「順位/競合ダッシュボード（P3-04）」を確認する
5. 必要なら「ダッシュボード再読込」を押す

### 期待結果
- 最新runサマリー（平均順位/最上位キーワード/競合平均順位）が表示される
- 主要キーワードの順位推移チャートが表示される
- 最新runの競合比較グラフ（順位）が表示される
- run別サマリーテーブルが表示される

### もし空表示になったら
- run履歴が0件の可能性が高いです。先に「収集実行（MOCK）」を1回実行してください
- 競合比較だけ空の場合は、競合ターゲットが未登録か、競合0件runです（仕様どおり）
- エラー表示の場合はスクショを送ってください（こちらで切り分けます）

## （P3-05）NAP整合性チェックを使う手順
「順位計測」画面で、店舗NAP（名前/住所/電話）と媒体設定値のズレをチェックできます。

### 手順1: migration適用
1. Supabaseの「SQL Editor」を開く
2. `supabase/migrations/202602060018_p3_nap_consistency_check.sql` を貼り付けて実行
3. `Success. No rows returned` を確認する
...
2. 右上の店舗セレクタで対象店舗を選ぶ
3. 左メニューの「順位計測」を開く
4. 「NAP整合性チェック（P3-05）」の「NAPチェック実行」を押す
5. 履歴から最新runを選んで、詳細を確認する

期待結果:
- run履歴が追加される
- `MATCH / MISMATCH / MISSING` が表示される
- `MISMATCH` の場合は不一致項目（name/address/phone）が表示される

### 注意
- provider設定が未入力の場合は `MISSING` になります（仕様）
- 失敗時はスクショを送ってください。こちらで原因切り分けします

## （P3-06）NAPアラート運用を使う手順
NAP整合性チェック（P3-05）の結果から、未対応アラートを一覧化し、`ACK/解消` を管理できます。

### 手順1: migration適用
1. Supabaseの「SQL Editor」を開く
2. `supabase/migrations/202602060019_p3_nap_alert_operations.sql` を貼り付けて実行
3. `Success. No rows returned` を確認する
...
2. 右上の店舗セレクタで対象店舗を選ぶ
3. 左メニューの「順位計測」を開く
4. `P3-05` の「NAPチェック実行」を押す
5. 「NAPアラート（P3-06）」で一覧を確認する

期待結果:
- `MISMATCH/MISSING` のproviderが未対応アラートとして表示される
- `MATCH` のproviderは未対応一覧に表示されない

### 手順3: ACK/解消
1. アラートで `ACK` を押す
2. 再読込して `ACKED` が維持されることを確認する
3. `解消` を押す

期待結果:
- `ACK` で `OPEN -> ACKED` に変わる
- `解消` で未対応一覧から消える

### もし表示できなかったら
- `P3-06 migration（NAP alert operations）の適用後に再試行してください。` の場合: migration未適用です（P3-05は継続可能）
- それ以外: スクショを送ってください（こちらで切り分けます）
